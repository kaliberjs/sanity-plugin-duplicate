...
exports.DocumentActionDuplicate = DocumentActionDuplicate;
...
function DocumentActionDuplicate(_ref) {
  var type = _ref.type,
  ...
  return {
    ...
    onHandle: function () {
      var _onHandle = _asyncToGenerator(function* () {
        ...
        var replacementFunctions = getReplacementFunctions(_schema.default.get(type));
...
function getReplacementFunctions(schema) {
  return Object.fromEntries(schema.fields.filter(x => {
    var _x$type$kaliberOption;

    return ![undefined, null].includes((_x$type$kaliberOption = x.type.kaliberOptions) === null || _x$type$kaliberOption === void 0 ? void 0 : _x$type$kaliberOption.duplicate);
  }).map(field => [field.name, field.type.kaliberOptions.duplicate])); // return Object.fromEntries(schema.fields
  //   .filter(x =>
...
//# sourceMappingURL=DocumentActionDuplicate.js.map